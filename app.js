document.addEventListener('DOMContentLoaded', () => {
    ...
    async function updateAllData() {
        try {
            ...
            // Deduplicate by Name (Smart Merge for Cracked Servers)
            const deduped = {};
            rawPlayers.forEach(p => {
                const name = p.username.toLowerCase();
                // Priority: Keep Online first, then keep higher stats/playtime
                ...
                if (!existing) {
                    ...
                    if (isNewActive || (!existing.online && hasMoreStats)) {
                        deduped[name] = p;
                    }
                }
            });
